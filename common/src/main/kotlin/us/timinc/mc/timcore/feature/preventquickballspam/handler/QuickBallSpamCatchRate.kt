...
object QuickBallSpamCatchRate {
    fun handle(evt: PokemonCatchRateEvent, @Suppress("unused") config: FeatureConfig, logger: Logger) {
        ...
        if (reversionFactor == 1F) {
            logger.sing("Ball effect not applied (usually due to being used outside of battle). No need to revert.")
            return
        }
